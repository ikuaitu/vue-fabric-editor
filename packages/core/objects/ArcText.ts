...
export default fabric.util.createClass(fabric.IText, {
    type: 'arc-text',
    ...
    renderCharCallback: function (method: any, ctx: CanvasRenderingContext2D, lineIndex: number, charIndex: number, endCharIndex: number, left: number, top: number, fullDecl: any) {
    ...
    runCharRendering: function (method: any, ctx: CanvasRenderingContext2D, _char: string, left: number, top: number, angle: number, fullDecl: any, alignment?: string) {
        if (ctx) {
            ctx.save();
    ...
    initDimensions: function () {
        ...
        for (let i = 0; i < this.__charBounds.length; i++) {
            ...
            for (let j = 0; j < row.length; j++) {
                ...
                if (ct.char?.trim() && bounds.contour) {
                    let cos = (util as any).cos(-charAngle), sin = (util as any).sin(-charAngle);
                    ...
                    if (curvature > 0) {
                        ...
                        ct.contour.tr = (util as any).transformPoint({ x: x + ct.contour.w, y: -y - ct.contour.h }, matrix);
                    } else {
                        const x = - ct.contour.x + this.__charBounds[i][j].width / 2
                        ct.contour.br = (util as any).transformPoint({ x: x - ct.contour.w, y: y }, matrix);
                        ct.contour.bl = (util as any).transformPoint({ x: x, y: y }, matrix);
        ...
        for (let i = 0; i < cts.length; i++) {
            const ctsl = cts[i], cta = ctsl[0], ctb = ctsl[ctsl.length - 1]
            let bbox, bbox2
            if (curvature > 0) {
    ...
    interateTextChunks: function (lineIndex: number, foo: Function, iteratorFn?: Function) {
    ...
    _drawTextLinesDecorationSector: function (ctx: CanvasRenderingContext2D, currentColor: string, offset: number, line: number, charStart: number, charEnd: number) {
    ...
    _contextSelectBackgroundSector: function (ctx: CanvasRenderingContext2D, line: number, charStart: number, charEnd: number, fullLineRadius?: boolean) {
        ctx.beginPath()
        let startChar = this._charTransformations[line][charStart];
